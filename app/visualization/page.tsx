...
function MainContent() {
    ...
    async function fetchRegions() {
      ...
      if (Array.isArray(data)) {
        data.forEach((item: any) => {
          ...
          if (
            item.province_code &&
            !regions.some((r) => r.region_code === item.province_code)
          ) {
            regions.push({
          ...
          if (
            item.city_code &&
            !regions.some((r) => r.region_code === item.city_code)
          ) {
            regions.push({
          ...
          if (
            item.district_code &&
            !regions.some((r) => r.region_code === item.district_code)
          ) {
            regions.push({
  ...
  const cityList = selectedProvince
    ? regions.filter(
        (r) =>
          r.region_level === "2" &&
          r.parent_code === selectedProvince.region_code
      )
    : [];
  // 区县列表（region_level === '3'，parent_code === 选中城市的 region_code）
  const countyList = selectedCity
    ? regions.filter(
        (r) =>
          r.region_level === "3" && r.parent_code === selectedCity.region_code
      )
    : [];
  ...
  const fetchRealData = async (
    ...
    const allSameCityCounties = regions.filter(
      (r) =>
        r.region_level === "3" && r.parent_code === selectedCity.region_code
    );

    const allRegionCodes = allSameCityCounties.map(
      (county) => county.region_code
    );
    ...
    const results = await Promise.all(
      allRegionCodes.map(async (region_code) => {
        ...
        const res = await fetch(
          `/api/visualization?region_code=${region_code}&year_dim=${year_dim}`
        );
        const { data } = await res.json();
        return data && data[0]
          ? {
              ...data[0],
              isUserRegion: region_code === selectedCounty.region_code,
            }
          : null;
      })
    );
    ...
    console.log(
      `=== ${selectedCity.region_name} 所有区县数据（按项目数量排序）===`
    );
    console.log(
      `统计周期: ${
        timeRange === "1year"
    ...
    if (userRegion) {
      ...
      const uniqueResult = Array.from(
        new Map(filtered.map((item) => [item.region, item])).values()
      );
  ...
  const generateInsights = (
    ...
    const userRank =
      sorted.findIndex((d) => d.region === userRegion.region) + 1;
    const topRegion = sorted[0];

    const insights: InsightData[] = [];
    insights.push({
      icon: <Crown className="h-5 w-5" />,
      text: `${selectedRegion}在对比中排名第${userRank}位`,
      type: "rank",
    });
  ...
  const CanvasBarChart = ({
    ...
    useEffect(() => {
      ...
      Promise.all([
        ...
        .then(([Chart, ChartDataLabels]) => {
          if (destroyed) return;
          // 优化：根据downloadMode动态调整参数
          const fontSize = downloadMode ? 14 : isMobile ? 10 : isFullscreen ? 28 : 20;
          const xLabelRotation = downloadMode ? 45 : isMobile ? 0 : 45;
          const paddingBottom = downloadMode ? 100 : 48;
          const dataLabelFontSize = downloadMode ? 13 : isMobile ? 12 : 20;
          const dataLabelOffset = downloadMode ? 12 : 4;
          const xLabelFontSize = downloadMode ? 14 : isMobile ? 10 : 24;
          const yLabelFontSize = downloadMode ? 14 : isMobile ? 10 : 24;
          const legendFontSize = downloadMode ? 16 : isMobile ? 10 : isFullscreen ? 28 : 20;
                ...
                datalabels: {
                  ...
                  formatter: (value: any, ctx: any) => {
                    if (downloadMode) {
                      ...
                      // 页面展示时单行
                      return ctx.dataset.label === "项目数量(个)"
                        ? `${value}个`
                        : `${value}亿元`;
                    }
                  },
    ...
    }, [data, isFullscreen, showDataLabels, downloadMode]);

    return (
      <div
        className={`${
          isFullscreen ? "h-[80vh]" : isMobile ? "h-60" : "h-[500px]"
        } w-full`}
      >
  ...
  const handleProvinceChange = (provinceCode: string) => {
    const province =
      provinceList.find((p) => p.region_code === provinceCode) || null;
  ...
  const isCityDisabled =
    !selectedProvince ||
    (selectedProvince &&
      DIRECT_MUNICIPALITY_CODES.includes(selectedProvince.region_code));

  // VisualizationPage 组件内，添加下载函数：
  const handleDownloadChart = () => {
    setShowDataLabels(true);
    setDownloadMode(true); // 开启下载模式
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* 主体内容 */}
      <main className="w-full px-2 py-4 sm:py-8">
            ...
            <div className="bg-white rounded-xl shadow-lg w-full p-2 sm:p-6 sticky top-4">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <Select
                    value={selectedProvince?.region_code || ""}
                ...
                <div>
                  ...
                  <Select
                    value={
                      selectedCity?.region_code ||
                      (selectedProvince &&
                      DIRECT_MUNICIPALITY_CODES.includes(
                        selectedProvince.region_code
                      )
                        ? selectedProvince.region_code
                        : "")
                    }
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      DIRECT_MUNICIPALITY_CODES.includes(
                        ...
                        <SelectItem
                          key={selectedProvince.region_code}
                          value={selectedProvince.region_code}
                        >
                  ...
                  >
                    ...
                    <SelectContent>
                      {countyList.map((county) => (
                        <SelectItem
                          key={county.region_code}
                          value={county.region_code}
                        >
                          {county.region_name}
                        </SelectItem>
                      ))}
            ...
            <div className="bg-white rounded-xl shadow-lg h-full w-full p-2 sm:p-8">
              {!hasGenerated && !isLoading && (
                <div className="text-center py-10 sm:py-20">
                  ...
                  <p className="text-xs sm:text-base text-gray-500 mb-6">
                    选择省份、城市、区县和时间范围，生成智能对比分析图表
                  </p>
                </div>
              )}
                ...
                <div className="space-y-4 sm:space-y-8">
                  ...
                  <div className="flex flex-col items-center sm:grid sm:grid-cols-3 gap-4 sm:gap-6 w-full">
                    {insights.map((insight, index) => (
                      ...
                      >
                        <div className="text-blue-600 text-2xl flex-shrink-0 flex items-center justify-center mb-1">
                          {insight.icon}
                        </div>
      ...
      <Dialog open={isFullscreen} onOpenChange={setIsFullscreen}>
        <DialogContent
          className="max-w-[95vw] max-h-[95vh] p-6"
          hideCloseButton
        >
          <DialogHeader>
            ...
            <DialogDescription className="text-xs sm:text-base text-black">
              详细的区县项目数量和发行规模对比分析图表
            </DialogDescription>
          </DialogHeader>
          {/* 右上角绝对定位的退出全屏按钮 */}
          <Button
      ...
      {showQR && typeof window !== "undefined" && (
        ...
        >
          ...
          >
            ...
            <div>
              <Button onClick={handleCloseQR} style={{ marginTop: 16 }}>
                关闭
              </Button>
            </div>
          </div>
