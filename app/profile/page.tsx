    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        >
          ...
          <h1 className="text-2xl font-light text-slate-800 tracking-wide">个人中心</h1>
        </motion.div>
            ...
            <CardContent className="p-6">
              <div className="space-y-8">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <p className="text-xl font-light text-slate-800 mb-1 tracking-wide">{user?.name || "加载中..."}</p>
                    <p className="text-base text-slate-500 font-light">
                      {user?.phone ? formatPhone(user.phone) : "加载中..."}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4 text-base">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      ...
                      <span className="ml-2 font-medium text-slate-800">{user?.employeeId || "加载中..."}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 font-light">角色：</span>
                      <span className="ml-2 font-medium text-slate-800">{user?.role || "加载中..."}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 font-light">上级主管：</span>
                      <span className="ml-2 font-medium text-slate-800">{user?.manager || "加载中..."}</span>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <span className="text-slate-500 font-light">所属团队：</span>
                      <span className="ml-2 font-medium text-slate-800">{user?.team || "加载中..."}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 font-light">负责区域：</span>
                      <span className="ml-2 font-medium text-slate-800">
                        {user?.regions?.join("、") || "加载中..."}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        >
          <Card className="bg-white border border-gray-200 shadow-sm rounded-lg">
            <CardHeader className="pb-4">
              <CardTitle className="text-lg font-light text-slate-800 tracking-wide">账号安全</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <div className="flex items-center justify-between py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        {user?.wechatId ? `已绑定：${user.wechatId}` : "绑定微信号可以接收重要通知"}
                      </p>
                    </div>
                  </div>
                  <Dialog open={isWechatDialogOpen} onOpenChange={setIsWechatDialogOpen}>
                    ...
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        <DialogTitle>{user?.wechatId ? "管理微信绑定" : "绑定微信号"}</DialogTitle>
                      </DialogHeader>
                      <div className="space-y-4">
                        {user?.wechatId ? (
                          <div className="space-y-4">
                            <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                              <p className="text-sm text-green-800">
                                当前已绑定微信号：<span className="font-medium">{user.wechatId}</span>
                              </p>
                            </div>
                          ...
                          <div className="space-y-4">
                            ...
                            <p className="text-xs text-slate-500 text-center">绑定后可以接收系统通知和重要消息提醒</p>
                          </div>
                        )}
                      </div>
                    </DialogContent>
