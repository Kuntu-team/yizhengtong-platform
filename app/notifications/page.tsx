...
import { Bell, User, FileText, ChevronLeft, Star, CheckCircle } from "lucide-react"
...
const mockNotifications: Notification[] = [
  ...
  // 添加更多通知以展示完整列表
  {
    ...
    type: "new_policy",
    title: "制造业数字化转型行动方案",
    description: "工信部发布制造业���字化转型行动方案，推动传统制造业向智能制造转型",
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between mb-2">
            <h3 className={`font-medium text-slate-800 line-clamp-1 ${!notification.read ? "font-semibold" : ""}`}>
              {notification.title}
            </h3>
            <div className="flex items-center gap-2 ml-2">
              {!notification.read && <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse" />}
              <span className="text-xs text-slate-500 whitespace-nowrap">{notification.timeAgo}</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Badge
                variant={notification.type === "leader_news" ? "default" : "secondary"}
            ...
            {notification.read && <CheckCircle className="h-4 w-4 text-green-500" />}
          </div>
        </div>
      </div>
    </motion.div>
        ...
        <div className="h-16 px-6 flex items-center justify-between max-w-6xl mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-light text-slate-800 tracking-wide">通知中心</h1>
              {unreadCount > 0 && (
                <motion.span
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
          ...
          {unreadCount > 0 && (
            <Button variant="ghost" size="sm" onClick={handleMarkAllAsRead} className="font-light hover:bg-white/20">
              全部已读
            </Button>
          )}
        </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="w-full h-12 rounded-none bg-transparent border-0 p-0">
              ...
              <TabsTrigger
                ...
                className="flex-1 data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-blue-500 data-[state=active]:text-blue-600 data-[state=active]:font-semibold rounded-none h-full text-base font-medium text-slate-700"
              >
                关注动态 ({notifications.filter((n) => n.type === "leader_news").length})
              </TabsTrigger>
              <TabsTrigger
                value="policy"
                className="flex-1 data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-blue-500 data-[state=active]:text-blue-600 data-[state=active]:font-semibold rounded-none h-full text-base font-medium text-slate-700"
              >
                新政策 ({notifications.filter((n) => n.type === "new_policy").length})
              </TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
