...
export async function GET(req: NextRequest) {
  try {
    ...
    const followedPersonIds = followed.map((f) => f.followed_person_id).filter((id): id is string => id !== null);
    ...
    console.log("Error in business-person-news API:", error);
    return NextResponse.json(
      { error: "Failed to fetch business person news" },
      { status: 500 }
    );
  }
}
